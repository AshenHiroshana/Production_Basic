...
export class VehicleTableComponent extends AbstractComponent implements OnInit {
  ...
  vehiclestatusField = new FormControl();
  ...
  async loadData(): Promise<any> {
    ...
    pageRequest.addSearchCriteria('vehiclestatus', this.vehiclestatusField.value);
  ...
  setDisplayedColumns(): void{
    this.displayedColumns = ['photo', 'no', 'brand', 'model', 'type', 'status'];
  ...
  async delete(vehicle: Vehicle): Promise<void>{
    const dialogRef = this.dialog.open(DeleteConfirmDialogComponent, {
      width: '300px',
      data: {message: vehicle.no + ' - ' + vehicle.vehicletype }
    });
